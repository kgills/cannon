...
int main(int argc, char *argv[])
{
    ...
    float* restrict matrix_A[ARRAY_DIM];
    float* restrict matrix_B[ARRAY_DIM];
    float* restrict matrix_C[ARRAY_DIM];
    for(i = 0; i < ARRAY_DIM; i++) {

        matrix_A[i] = (float* restrict)malloc(ARRAY_DIM * sizeof(float));
        matrix_B[i] = (float* restrict)malloc(ARRAY_DIM * sizeof(float));
        matrix_C[i] = (float* restrict)malloc(ARRAY_DIM * sizeof(float));
    ...
    #pragma acc kernels loop copyin(matrix_A[:ARRAY_DIM][:ARRAY_DIM], matrix_B[:ARRAY_DIM][:ARRAY_DIM]) copyout(matrix_C[:ARRAY_DIM][:ARRAY_DIM])
    for(i = 0; i < ARRAY_DIM; i++) {
        #pragma acc loop
        for(j = 0; j < ARRAY_DIM; j++) {
            float sum = 0;
